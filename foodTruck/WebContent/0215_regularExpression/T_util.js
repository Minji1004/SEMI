...
function numberWithCommas(n) {
    var parts = n.toString().split(".");   // 소수자리는 ,들어가면안되니까. 
    parts[0] = parts[0].replace(/\B(?=(\d{3})+(?!\d))/g, ",");  // parts[0] : 정수부.
	    // \B : 단어의 비경계 = 공백의 위치 (없으면 맨앞에도 , 들어감.)   12ab12 -> \B      ab12  -> \b
	    // (?=(\d{3})+  : 3자리정수 1개이상.
	    // (?!\d))  : 정수아닌문자열.    123,12
    return parts.join(".");   // 배열 결합해서 출력 결합사이 -> .
}
